...
class LbportfolioModelLbportfolioitemlist extends JModel{
	/**
	 * Lbportfolioitemlist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	function __construct(){
        ...
        $cat_id = $app->getUserStateFromRequest('item.cat_id', 'cat_id', 0, 'int');
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__lbportfolio_item` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,name,description,url,published,img'); // array('id', 'ordering', 'published'); 

		// retrive ordering info
		$filter_order = $app->getUserStateFromRequest('com_lbportfoliofilter_order', 'filter_order', $default_order_field);
		$filter_order_Dir = strtoupper($app->getUserStateFromRequest('com_lbportfoliofilter_order_Dir', 'filter_order_Dir', 'ASC'));
	...
	private function _buildQueryWhere() {
		...
		$search = $app->getUserStateFromRequest('com_lbportfoliosearch', 'search', '');
		$cat_id = $app->getUserStateFromRequest('item,cat_id', 'cat_id', 0, 'int');
		...
		if ($search) {
			$allowedSearch = explode(',', 'name,url,published,img'); // array('id', 'ordering', 'published'); 
			$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__lbportfolio_item` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__lbportfolio_item` ORDER BY `$fieldName`');
		$options = array();
	...
	public function getCategories(){
			...
			$db =& JFactory::getDBO();
			$query = 'SELECT * FROM `#__lbportfolio_cat`';
			$db->setQuery($query);
			$categoryList = $db->loadAssocList();
			return $categoryList;
	}
