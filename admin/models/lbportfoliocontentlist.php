 ...
 */
class LbportfolioModelLbportfoliocontentlist extends JModel{
	/**
	 * Lbportfoliocatlist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__lbportfolio_content` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,name,published,description'); // array('id', 'ordering', 'published'); 
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', 'name,published'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__lbportfolio_content` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__lbportfolio_content` ORDER BY `$fieldName`');
		$options = array();
