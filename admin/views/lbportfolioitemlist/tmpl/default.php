...
<?php 
	...
	if ($categories) {
		foreach ($categories as $key => $val) {
			$cat[] = JHtml::_('select.option', $val['id'], $val['name']);
		}
	}
...
<div id="editcell">
		...
		<tr>
			...
			<td align="left" width="25%">
				<select name="cat_id" class="inputbox" onchange="this.form.submit()">
					<option value=""><?php echo JText::_( 'Chose Filter Category' ); ?></option>
	...
	for ($i=0, $n=count( $this->rows ); $i < $n; $i++)	{
		...
		$link = JRoute::_( 'index.php?option=com_lbportfolio&controller=lbportfolioitemlist&task=edit&cid[]='. $row->id);
		?>
...
<input type="hidden" name="controller" value="lbportfolioitemlist" />
