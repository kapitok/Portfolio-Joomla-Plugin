 ...
 */
class LbportfolioControllerLbportfoliocontentlist extends LbportfolioController{
	...
	public function edit(){
		JRequest::setVar( 'view', 'lbportfoliocontent' );
		JRequest::setVar( 'layout', 'default'  );
		JRequest::setVar( 'hidemainmenu', 1 );
		
		$view =& $this->getView('lbportfoliocontent', 'html');
	...
	public function save(){
		$model = $this->getModel('lbportfoliocontentlist'); 
		...
		$link = 'index.php?option=com_lbportfolio&controller=lbportfoliocontentlist';
	...
	public function remove(){
		$model = $this->getModel('lbportfoliocontent');
		if(!$model->delete()) {
		...
		$this->setRedirect( 'index.php?option=com_lbportfolio&controller=lbportfoliocontentlist', $msg );
	}
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__lbportfolio_content SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_lbportfolio&controller=lbportfoliocontentlist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__lbportfolio_content SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
